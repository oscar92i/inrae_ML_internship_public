...
def evaluate_ConvTran_binary_batch(data, metadata, train_split_size:float, validation_split_size:float, n_splits=5, n_epochs=1000, lr=1e-3, early_stopping_rounds=20, batch_size=64):
    ...
    for seed in range(n_splits):
        ...
        X_train, X_val, X_test, y_train, y_val, y_test = train_validation_test_split_by_plotid(
            data, metadata, train_split_target=train_split_size, validation_split_target=validation_split_size, seed=seed, print_len_splits=True
        )
        ...
        y_test_pred = torch.cat(all_preds).cpu().numpy()
        y_test_true = torch.cat(all_trues).cpu().numpy()
    ...
    print(f'\nConvTran Binary Classification Batch {batch_size}')
    print(f'Mean Accuracy: {np.mean(accs):.3f} ± {np.std(accs):.3f}')
    print(f'Mean F1 Score: {np.mean(f1s):.3f} ± {np.std(f1s):.3f}')
